...
TEMPLATE_FIELDS = [
    ...
    {
        "field_name": "末帧模式",
        "type": 3,  # 单选：决定最后一帧/中间帧的处理方式
        "property": {
            "options": [
                {"name": "标准"},      # 末尾无字幕、非黑屏：直接用最后一帧，不去字幕
                {"name": "去字幕"},    # 末尾有字幕：调生图模型去字幕
                {"name": "黑屏渐显"},  # 末尾渐隐黑屏：无中间帧无配音，末段渐显图层
            ]
        },
    },
    {
        "field_name": "渐显时长",
        "type": 2,  # 数字：黑屏渐显模式下末尾渐显的秒数，默认 4
    },
    {
        "field_name": "配音音色",
        "type": 3,  # 单选
    ...
    {
        "field_name": "调试信息",
        "type": 1,  # 文本：每步耗时/TTS并发/尾帧缓存/转场拼接响应，替代扣子运行时拿不到的日志
    },
]
